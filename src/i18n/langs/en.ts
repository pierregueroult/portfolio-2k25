import type { Texts } from "../index";

export const en: Texts = {
  components: {
    header: {
      lang: "Language",
      theme: "Theme",
      paris: "Paris",
      france: "France",
    },
    navigation: {
      home: "Home",
      socials: "Socials",
      projects: "Projects",
      blog: "Blog",
    },
    theme: {
      default: "Default themes",
      special: "Special themes",
      search: "Search a theme",
    },
    lang: {
      choose: "Choose a language",
      current: "current",
    },
    modal: {
      close: "Close",
    },
    items: {
      more: "Read more",
      now: "right now",
      from: "From",
      to: "to",
      about: "about",
      work: {
        looking: "Looking for",
        near: "near",
      },
      certification: {
        certifiedBy: "Certified by",
      },
    },
    noTranslation: {
      title: "Translation in progress...",
      text: "This page is not yet available in English, but I'm working on it! In the meantime, you can read it in French or come back later for the translated version.",
    },
    contact: {
      name: "Name",
      email: "Email",
      message: "Write down your message here...",
      send: "Send",
      consent: "By submitting this form, you agree to be contacted by email. See the ",
      privacy: "privacy policy",
      consent2: "for more information.",
    },
  },
  pages: {
    blog: {
      meta: {
        title: "Blog",
        description: "Read my latest articles, thoughts, and tech tutorials. I write about web development, software engineering, automation, and tech culture.",
        keywords: "blog, articles, thoughts, tutorials, web development, software engineering, automation, tech culture",
      },
      title: "Blog - Insights & Experiments",
      subtitle: "Exploring web development, automation, and new technologies—one project at a time.",
      articles: "Articles",
      translation: {
        title: "Translation in progress...",
        text: "These articles are not yet available in English, but I'm working on it! In the meantime, you can read them in French or come back later for the translated version.",
      },
    },
    socials: {
      meta: {
        title: "Connect with me",
        description: "Find and follow me across social media and professional networks. Stay updated on my latest projects, blog posts, and tech insights.",
        keywords: "socials, social media, professional networks, projects, blog posts, tech insights",
      },
      title: "My Socials - Let's Stay Connected",
      subtitle: "Find me on social and professional platforms to follow my projects and connect over web development and automation.",
      featured: "Featured",
      links: "Social Links",
      contact: {
        title: "Send me a message",
        description: "Have a question, a project, or just want to chat, feel free to reach out!",
      },
      music: {
        title: "What I'm listening to",
        description: "Here's what I'm currently listening to on Spotify. Maybe you'll find something you like!",
        spotify: "Open on Spotify",
        cover: "'s Album Cover",
      }
    },
    projects: {
      meta: {
        title: "Projects & Works",
        description: "Explore my latest web development projects, automation tools, and tech experiments. Always building, always learning.",
        keywords: "projects, works, web development, automation tools, tech experiments",
      },
      title: "Projects, Creations and contributions",
      subtitle: "Here are some of the projects I've led, built, or contributed to. I focus on creating developer tools, automation solutions, and innovative web applications that improve efficiency, streamline workflows, and push the boundaries of technology.",
      creations: "Creations",
      projects: "Projects",
      contributions: "Contributions",
      schools: "School Projects",
      translation: {
        title: "Translation in progress...",
        text: "These project pages are not yet available in English, but I'm working on it! In the meantime, you can read them in French or come back later for the translated version.",
      },
    },
    index: {
      meta: {
        title: "Web & Software Developer",
        description: "20-year-old Web Developer & Computer Science Student passionate about building sleek, high-performance websites and automating workflows. Always exploring new technologies to solve problems and optimize efficiency.",
        keywords: "web developer, software developer, computer science student, web development, automation, new technologies",
      },
      title: "Full-Stack Software Engineer - Pierre Guéroult - Paris, France",
      subtitle: "20 year-old web developer & Computer Science Student. I love crafting sleek websites and automating workflows to make life easier. Passionate about clean code, problem-solving, and making tech work smarter, not harder. Always exploring new technologies, always up for a challenge. Whether it's web development, automation, or something completely out of my comfort zone - I'm in.",
      works: "Works",
      schools: "Schools",
      certifications: "Certifications",
      curriculum: "Download my Curriculum Vitae",
    },
  },
};
